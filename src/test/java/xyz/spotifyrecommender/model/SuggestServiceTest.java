...
public class SuggestServiceTest {
  ...
  public void notEnoughDataIfNoSongsAvailable() {
    doReturn(new TopTracksDTO()).when(spotifyAPI)
        .getTopTracks(anyString(), anyString(), anyString());

    RecommendationDTO recommendations = suggestService.getRecommendations(token, "false", "true");
  ...
  public void songsAreReplacedIfOldSongsStillPresent() {
    doReturn(Arrays.asList(new TrackURI())).when(spotifyAPI)
  ...
  public void noRecommendationsIfSpotifyGivesNoRecommendations() {
    RecommendationDTO recs = new RecommendationDTO();
    recs.setTrackList(Collections.emptySet());
    doReturn(recs).when(spotifyAPI).getRecommendations(anyList(), anyString());
  ...
  public void listIsSteppedBySpecifiedSize() {
    ...
    List<String> resultList = suggestService.getListByStepSize(itemList);
