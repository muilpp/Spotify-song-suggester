...
public class UserDAOImpl implements UserDAO {
    ...
    public boolean addUser(String userName, String accessToken, String refreshToken) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            User user = new User(userName, accessToken, refreshToken, DEFAULT_ACCESS_REVOKED, DEFAULT_AVOID_SPANISH_MUSIC, DEFAULT_SHORT_TERM_TRACKS, LocalDateTime.now());
    ...
    public boolean updateUserAccess(String userName, String accesRevoked, String newAccessToken,
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (!accesRevoked.equalsIgnoreCase("1")) {
                ...
                query = session
                        .createQuery("update User set access_revoked = :accesRevoked, updated_at = :currentTime where user_name = :userName");
    ...
    public boolean updateUserLastRefresh(String userName, LocalDateTime lastRefreshTime) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            query = session.createQuery("update User set updated_at = :lastRefreshTime where user_name = :userName");
